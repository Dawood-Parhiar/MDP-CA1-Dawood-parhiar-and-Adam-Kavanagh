...
{
	return !m_player_ship->IsMarkedForRemoval();
}

bool World::HasPlayerReachedEnd() const
{
	return !m_world_bounds.contains(m_player_ship->getPosition());
}

void World::LoadTextures()
{
	m_textures.Load(TextureID::KPirate, "Media/Textures/Eagle.png");
...
void World::BuildScene()
{
	...
	std::unique_ptr<PirateShip> leader(new PirateShip(ShipType::kPirateShip, m_textures, m_fonts));
	m_player_ship = leader.get();
	m_player_ship->setPosition(m_spawn_position);
	m_player_ship->SetVelocity(40.f, m_scrollspeed);
	...
	m_player_ship->AttachChild(std::move(left_escort));
	...
	m_player_ship->AttachChild(std::move(right_escort));*/
}
...
void World::AdaptPlayerPosition()
{
	...
	sf::Vector2f position = m_player_ship->getPosition();
	position.x = std::max(position.x, view_bounds.left + border_distance);
	position.x = std::min(position.x, view_bounds.left + view_bounds.width - border_distance);
	position.y = std::max(position.y, view_bounds.top + border_distance);
	position.y = std::min(position.y, view_bounds.top + view_bounds.height -border_distance);
	m_player_ship->setPosition(position);
}

void World::AdaptPlayerVelocity()
{
	sf::Vector2f velocity = m_player_ship->GetVelocity();

	//If they are moving diagonally divide by sqrt 2
	if (velocity.x != 0.f && velocity.y != 0.f)
	{
		m_player_ship->SetVelocity(velocity / std::sqrt(2.f));
	}
	//Add scrolling velocity
	m_player_ship->Accelerate(0.f, m_scrollspeed);
}
...
void World::SpawnEnemies()
{
	...
	while (!m_enemy_spawn_points.empty() && m_enemy_spawn_points.back().m_y > GetBattleFieldBounds().top)
	{
		...
		std::unique_ptr<PirateShip> enemy(new PirateShip(spawn.m_type, m_textures, m_fonts));
...
void World::GuideMissiles()
{
	...
	enemyCollector.action = DerivedAction<PirateShip>([this](PirateShip& enemy, sf::Time)
		{
	...
	missileGuider.action = DerivedAction<Projectile>([this](Projectile& missile, sf::Time dt)
		{
			...
			PirateShip* closest_enemy = nullptr;

			for (PirateShip* enemy : m_active_enemies)
			{
...
void World::HandleCollisions()
{
	...
	for (SceneNode::Pair pair : collision_pairs)
	{
		...
		{
			auto& player = static_cast<PirateShip&>(*pair.first);
			auto& enemy = static_cast<PirateShip&>(*pair.second);
			//Collision response
		...
		{
			auto& aircraft = static_cast<PirateShip&>(*pair.first);
